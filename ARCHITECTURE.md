...
- **A follow-up round specifically tested the parts the first prototype didn't cover** — whether haiku decides to escalate *on its own* (not just when explicitly told to), whether the escalation actually runs as sonnet, whether the research is real, and whether a real (safely reversible) write works — using a genuinely ambiguous task ("when do the Capitals next play in Tampa? add it to the calendar") rather than a scripted instruction, with a haiku `-p` invocation and a system prompt establishing the same judgment rule as everywhere else in this doc (§10a-1's SKILL.md, this section's own design): call tools directly when confident, spawn a sonnet research subagent for anything requiring a real-world fact, never fabricate a checkable detail.
  - **The judgment call itself works.** Haiku's own `thinking` block reasoned through the system prompt's rule unprompted ("I should not guess... I should spawn a subagent to research this fact") and issued a real `Agent` tool call — confirmed via the same "distinct task_started event, separate thinking block, real independent execution" evidence bar as the first round.
  - **Real gap found, not previously known: the sonnet model override doesn't reliably happen.** The system prompt asked for a sonnet-model subagent; haiku's actual `Agent` tool `input` contained no `model` field at all, and the subagent's own messages confirm it ran as `claude-haiku-4-5-20251001`, not sonnet. Haiku correctly decided *to* delegate, then silently dropped the *which model* part of the instruction. The subagent still did genuinely real research in this instance (three real `WebSearch` calls, correctly reconciled a real-world detail — an arena rename mid-search — rather than ignoring it) and landed on a plausible, internally-consistent answer, so this specific run wasn't harmed by running as haiku instead of sonnet — but one correct outcome isn't evidence the model tier is reliable for harder research generally, which was the entire reason to specify sonnet in the first place. **Whatever actually builds §10 needs to not rely on the LLM remembering to set `model` in its own tool call** — either the system prompt needs to be far more forceful/repeated about it, or (more robust) something needs to validate the `Agent` call's `input.model` and reject/retry if it's missing, rather than trusting instruction-following alone.
  - **The write path works end-to-end**, tested twice: once as an unplanned real consequence of the judgment-call test (haiku's research concluded correctly and it went ahead and called `create_event` for real, unprompted — exactly the intended behavior, just not the test that was nominally being run), and once as a deliberate create → verify → delete → verify round-trip. Both real production writes were independently reverified gone afterward (`list_events` swept directly against `backend/dist/actions/events.js`, not just trusting either agent's own report) — zero leftover test data.
- **A third round specifically fixed the model-override gap round two found**, since asking the system prompt more forcefully was never going to be reliable — the fix moves model selection out of the LLM's own tool-call parameters entirely, into deterministic orchestration:
  - **Mechanism:** haiku's `--allowedTools` doesn't include `Agent`/`Task` at all — it's structurally incapable of spawning its own subagent, so there's nothing for it to get wrong. Instead it's forced into a structured `--json-schema` response (`{needs_research, action_taken}`), which Claude Code enforces at the harness level (injecting a "you must call the StructuredOutput tool" nudge if the model doesn't do so proactively) — a materially more robust parse target than scraping a prose `result` string. When `needs_research` is set, the calling code launches a **fully separate** `claude -p "<question>" --model sonnet --allowedTools "WebSearch,WebFetch"` invocation — `--model` is a CLI flag on a call the orchestration code makes, not a parameter an LLM has to remember to set inside its own tool call, so it's unconditionally guaranteed correct.
  - **Chose "sonnet finishes the job itself"** over "sonnet's finding gets serialized back to a second haiku call" — both were tested and both work, but finishing on sonnet (given the same MCP tools) means one fewer re-serialization hop for the researched fact to get reworded or lost across, and the latency premium of finishing on sonnet only applies to the already-rare, already-slow research path anyway.
  - **Verified this actually enforces sonnet, not just "worked once":** every single assistant turn's `message.model` field (checked per-turn in the raw trace, across two independent research runs) was `claude-sonnet-5` — zero haiku turns doing the actual work. Real research activity confirmed too (5–13 genuine `WebSearch`/`WebFetch` calls per run, cross-checking multiple real sources). Triage was checked 4/4 times: every needs-research response had `action_taken: null` (no fabricated tool calls alongside a research flag), and the direct-action fast path (an unambiguous request) still correctly bypassed research entirely.
  - **A real methodological trap found along the way, worth remembering for any future check like this:** the top-level `modelUsage` field on a response is misleading on its own — even a pure `--model sonnet` call's `modelUsage` dict always contains a small `claude-haiku-4-5` entry from harness-internal overhead unrelated to the actual task. Naively checking "does modelUsage mention haiku" produces a false failure. The correct check is each assistant turn's own `message.model` field in the stream-json trace, not the aggregate summary.
  - **Two things worth carrying into the real implementation, found incidentally:** (1) system-prompt claims like "you don't have web search" are not a security boundary — haiku tried `WebSearch`/`WebFetch` anyway in every research-needed run despite being told it couldn't, and once even tried invoking the `calendar-add` Skill as a workaround; every attempt was correctly blocked by `--allowedTools`, confirming that's the actual enforcement point and prompt wording never is. (2) The one real write in this round required a self-correction — sonnet's first `create_event` call used wrong field names, got a clean MCP validation error, and retried correctly on its own — but a real implementation shouldn't rely on that happening; it needs an explicit "on `isError`, retry once with the error appended" policy rather than trusting a model to notice and fix its own mistake.
  - **Latency for real research is worse than round two's rough estimate**, which was based on a trivial lookup task: real multi-source research ran 43–110s for the research step alone in this round, ~40s for the combined research+write call. A "researching…" UI state isn't a nice-to-have, it's required for this path to not feel broken.
...
- **M8 (v2) — Command layer.** *Corrected from the original plan* (see §10's own note) — not Ollama; a headless Claude Code invocation (`claude -p`, `CLAUDE_CODE_OAUTH_TOKEN` for subscription billing, M3's MCP server auto-discovered — no `--mcp-config` needed) with haiku doing pure triage (no `Agent`/`Task` tool access — structurally can't spawn its own subagent, forced into a `--json-schema`-validated `{needs_research, action_taken}` response) and, when research is needed, a fully separate `--model sonnet` invocation the orchestration code launches directly — model selection is a CLI flag on a call the code makes, not a parameter any LLM has to remember to set correctly. Undo via the batch-tag mechanism generalized out of `calendar-add`, not a confirm-before-execute UI dialog. Three rounds of empirical prototyping now done (see §10 for full detail/evidence): round one confirmed the raw mechanism (real subagent execution, real MCP inheritance, the `--allowedTools`-required/silent-denial gotcha); round two confirmed haiku's own judgment call to escalate *unprompted* works and a real end-to-end write succeeds, but found the sonnet model override wasn't reliably set by the LLM itself; round three fixed that by moving model selection into deterministic orchestration instead of trusting instruction-following, and verified per-turn (not just aggregate) that sonnet is now unconditionally the one doing the research, across multiple independent runs. What's left before this actually ships: `POST /api/voice/command` wiring it all together, generalizing `calendar-add`'s batch-tag/undo logic into the shared action layer, the `voice_commands` audit table, frontend wiring beyond §9's current "display the raw transcript" scope, and an explicit decision on whether every action gets instant-execute-with-undo or whether some (deleting/moving an existing real event, specifically) still warrant a confirm step.
