...
- **M8 (v2) — Command layer.** *Corrected from the original plan* (see §10's own note) — not Ollama; a headless Claude Code invocation (`claude -p`, `CLAUDE_CODE_OAUTH_TOKEN` for subscription billing, M3's MCP server auto-discovered — no `--mcp-config` needed) with haiku as the fast default and an on-demand sonnet research subagent for anything needing real-world facts, same judgment call `calendar-add` already encodes. Undo via the batch-tag mechanism generalized out of `calendar-add`, not a confirm-before-execute UI dialog. Two rounds of empirical prototyping now done (see §10 for full detail/evidence, not just this summary): round one confirmed the raw mechanism (real subagent execution, real MCP inheritance, the `--allowedTools`-required/silent-denial gotcha); round two confirmed haiku's own judgment call to escalate *unprompted* actually works, confirmed a real end-to-end write (create → verify → delete → verify, both through the real MCP tools and independently reverified against the database directly), and found one real, unresolved gap worth fixing before this ships: haiku reliably decides *to* delegate but doesn't reliably set the sonnet model override on the `Agent` tool call it makes — something needs to validate/enforce that rather than trust instruction-following alone.
