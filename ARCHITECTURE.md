...
- **Fridge Android tablet — Fully Kiosk Browser setup** (per this section's existing kiosk-mode decision, nothing new to build in-app):
  1. Install Fully Kiosk Browser (free tier) from the Play Store.
  2. Point its start URL at the deployed dashboard URL; enable "Auto Launch on Boot" and "Motion Detection Screen Wake" (or disable screen timeout entirely, since it's a wall-mounted always-on display).
  3. Enable Fully Kiosk's own fullscreen/kiosk mode (hides Android system/status bars) — do **not** try to replicate this in-app, per this section's original decision.
  4. Enable Fully Kiosk's watchdog/auto-restart-on-crash setting so a rare tab crash doesn't leave the fridge display dead until someone notices.
  5. Confirm the tablet install banner (Chrome's native "Install app"/"Add to Home Screen" prompt) is not required for Fully Kiosk's use case — it's just browsing a URL in kiosk chrome, not installing the PWA the way the iPhone does; no action needed there.
  6. Leave it running for a few hours and confirm the §8b imminent-event pulse and SSE live-sync both keep working over a long-lived session (the tablet realistically never fully reloads once mounted).
