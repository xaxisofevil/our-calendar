...
- §10's command layer (now that it's corrected to run through Claude Code rather than a local Ollama model — see §10) is this same MCP server's *third* caller, not a fourth implementation: every headless `claude -p` invocation §10/M8 spawns auto-discovers this server (registered at User config scope, confirmed in §10's first prototyping round — no `--mcp-config` flag needed or used), so a voice command and a typed Claude Code request end up calling the exact same tools. The `calendar-add` skill (§10a-1) is the fourth — same reasoning, different trigger.
...
## 10b. Voice-Driven Auto-Confirm (M8a — done, on top of §10/M8's manual Confirm/Cancel)

Requested directly, with the "fully-instant auto-execute" alternative explicitly rejected: when a `needs_confirmation` result appears (§10's `proposedDestructiveAction` — deleting or updating an existing event/todo), the manual Confirm/Cancel buttons stay exactly as they were, but the mic *also* opens itself for up to 10 seconds, listening for a clear spoken yes/no, so confirming out loud is possible without a second tap. The whole point of this over the rejected instant-execute alternative is that an unclear signal must never auto-proceed — silence, a garbled clip, a low-confidence match, or ambiguous phrasing all fall back to doing nothing, leaving the manual buttons as the only way forward, exactly as before this feature existed.

**Why this is safe to auto-open the mic for, when it wasn't safe to auto-execute the action:** the two gates below (whitelist + confidence) only ever call the *existing* `confirmAction`/`cancelAction` — the same functions the manual buttons already call, going through the same `POST /api/voice/confirm` (execute) or pure client-side no-op (cancel) as before. Nothing new gets deleted or changed that a household member couldn't already have caused by tapping Confirm; auto-listen just adds a second, optional path to producing that same tap.

**Verified before building on top of it — programmatic `getUserMedia()` with no new user gesture:**
- **MDN's own documentation states plainly** that once a user has granted microphone permission for an origin, subsequent `getUserMedia()` calls succeed without re-prompting — permission is origin-scoped, not gesture-scoped, and there is no browser requirement that `getUserMedia()` itself be called from within a user-gesture handler (unlike, say, unmuted `<audio>`/`<video>` autoplay, which Chrome's own autoplay policy does gate on a gesture — a different API with a different rule, easy to conflate but confirmed distinct here).
- **Exercised directly in this project's own isolated e2e harness** (`e2e/voice-auto-confirm.spec.ts`): the auto-listen hook's `getUserMedia()` call fires the moment a `needs_confirmation` state appears, with zero synthetic pointer/click events dispatched between that state appearing and the call — proving the actual code path here doesn't rely on (or accidentally require) a fresh gesture, not just that the concept is supposed to work in principle.
- **The one real, sourced caveat, specific to iOS Safari standalone (home-screen) PWAs** — the mode this household's iPhone actually uses (§4/§4's iOS "Add to Home Screen" checklist): WebKit bug 185448 ("getUserMedia not working in apps added to home screen that run in standalone mode") was open for nearly two years and only fixed in iOS 13.4 (March 2020) — long since shipped, not a live concern on any iOS version this household would plausibly be running. A related, *still* only partially-resolved bug (215884) documents standalone PWA mode sometimes resetting mic permission (forcing a re-prompt) on a **hash-based navigation** — this app has no client-side routing at all (§4's own "single always-`/` URL" note), so that specific trigger doesn't apply here, but it's evidence that iOS Safari's standalone mode has been measurably less consistent about media-permission persistence than Chrome, historically and structurally, not just anecdotally.
- **Still genuinely open: a live iOS Safari device test of this exact feature.** The above is real, sourced evidence that the general mechanism works and roughly what's shaky about it on iOS specifically — it is not the same as watching this feature's own auto-listen mic actually open on Lindsay's iPhone in standalone mode. **Flagging this explicitly as an unverified item, not glossing over it**: do this once real-device testing is possible — trigger a `needs_confirmation` result on the installed iOS PWA and confirm the mic actually opens with no additional tap, the same manual checklist spirit as §4's own "What can't be automated" list.
- **Built defensively regardless of the above**, per explicit instruction: `useAutoListenConfirm.ts`'s `getUserMedia()` call is wrapped in a try/catch that does nothing but silently reset to idle on any failure — permission somehow unavailable, a browser quirk, anything at all. This is not a fallback that needs the iOS caveat above to be resolved first; it's already how every failure path in this feature behaves, iOS-specific or not. The manual Confirm/Cancel buttons never depend on auto-listen succeeding, so a worst-case "auto-listen silently never works on some browser" degrades this feature to exactly §10/M8's original manual-only behavior, never worse.

**The whitelist + confidence gate (`frontend/src/lib/voiceConfirmWhitelist.ts`) — why both are required, neither alone:**
- **Whitelist alone is unsafe** because Deepgram will confidently transcribe *something* for almost any sound — a dog bark, a TV in the background, a kid shouting something unrelated — and a naive substring/fuzzy match against "yes"/"no" risks matching noise that was never actually an answer to the question on screen.
- **Confidence alone is unsafe** because a high-confidence transcript of an *unrelated* sentence ("what's for dinner") is not a yes or no, no matter how clearly Deepgram heard it — confidence measures "how sure Deepgram is about the words," not "was this actually a response to the confirmation prompt."
- **Together**, they approximate "a household member clearly and deliberately said something from this specific short list, in direct response to what's on screen" — the actual thing this feature needs to detect before it's safe to auto-fire a destructive action with nobody's finger on a button.
- **Whitelist:** affirmative — "yes"/"yeah"/"yep"/"confirm"/"correct"/"do it"; negative — "no"/"nope"/"cancel"/"don't". Matched as whole words/short phrases (word-boundary, not substring — "sunny" never matches "no"), against Deepgram's own per-word tokens (`words[]`), not by re-splitting the smart-formatted transcript string, so there's no risk of a transcript-vs-word-array tokenization mismatch (punctuation, capitalization) silently breaking the match.
- **Confidence field/threshold:** Deepgram's pre-recorded `/v1/listen` response carries a 0–1 `confidence` double per word at `results.channels[].alternatives[].words[].confidence` (confirmed against Deepgram's own reference docs, not assumed/guessed — a distinct field from the utterance-level `alternatives[].confidence`, which this feature doesn't use). **Threshold: 0.8.** Deliberately conservative rather than a bare midpoint: a clearly-spoken common word like "yes"/"no" typically scores well above 0.9 in practice, while a misheard/ambiguous/background-noise word tends to fall well below this line — the gap is wide enough that 0.8 comfortably separates "clearly said" from "guessed." Since this decides whether to auto-fire a destructive action with nobody's finger on a button, erring toward requiring a cleaner signal (falling back to the always-available manual buttons on anything murkier) is the correct trade-off here, not a compromise to revisit later.
- A multi-word phrase ("do it") is only as trustworthy as its least-confident word — the match uses the *minimum* confidence across the phrase's words, not an average, so one clearly-said word next to one garbled one still fails the gate.
- Backend plumbing: `backend/src/lib/deepgram.ts`'s `transcribeAudio` now returns `{ transcript, words }` instead of a bare transcript string (its one caller, `routes/voice.ts`, updated to match — `POST /api/voice/transcribe`'s response gains a `words` field alongside the pre-existing `transcript`, additive and non-breaking for §9's original consumer). `e2e/mock-deepgram-server.mjs` mirrors this: every word gets a realistic-default 0.97 confidence unless a test opts into a specific value via a `CONF:<value>:<text>` prefix on the fake clip's bytes — the same "trigger substrings drive the mock's behavior" convention `TRIGGER_DEEPGRAM_ERROR`/`SILENT_CLIP` already established.

**Timing — 10s cap, ~1.3s silence-ends-it-early:** `frontend/src/lib/useAutoListenConfirm.ts` records for up to `MAX_RECORD_MS` (10,000ms) but stops sooner once real speech has been detected and then ~`SILENCE_MS` (1,300ms, the middle of the requested 1–1.5s range) of relative quiet follows — a natural pause after an answer, not a fixed always-wait-it-out timer. Silence detection is a basic Web Audio `AnalyserNode` RMS/amplitude check on a 100ms poll interval against a fixed threshold (`SPEECH_RMS_THRESHOLD`) — deliberately not real voice-activity-detection, per the brief's own "keep it simple" instruction; good enough to distinguish "someone is talking" from "background quiet" without needing anything more sophisticated. Falls back gracefully to the flat `MAX_RECORD_MS` timeout alone (no early-stop) if Web Audio isn't available or can't attach to the stream for any reason — never blocks the recording from happening, just loses the "end early" refinement.

A real, considered nuance worth recording: Chrome's autoplay policy auto-suspends an `AudioContext` created *before* the document has ever received a user gesture — but every auto-listen `AudioContext` here is created strictly *after* the real gesture that started the original press-and-hold recording moments earlier, so per Chrome's own documented behavior it should already initialize in the "running" state. `resume()` is still called defensively right after construction anyway (a harmless no-op on an already-running context) rather than assuming that holds on every browser without a fallback.

**A distinct hook, not a mode bolted onto `useVoiceCapture.ts`:** that hook's whole shape (`activeRef`/`pendingStopRef`, one `MediaRecorder` tied to `pointerdown`/`pointerup`) is built around a single press-and-hold session started by a user gesture. Auto-listen is a genuinely different mode — self-starting off a state change (not a gesture), time-boxed, silence-terminated, and evaluated against a narrow whitelist instead of driving the full command pipeline — so `useAutoListenConfirm.ts` is its own hook with its own `MediaRecorder` session, sharing only the browser APIs and `transcribeVoice` (`lib/api.ts`) with the press-and-hold path, not a second state machine threaded through code that has no other reason to know about it. `VoiceButton.tsx` guards against the two ever overlapping (a manual press is ignored while auto-listen's own mic session is active) and never lets a manual result get clobbered by an auto-listen attempt still in flight — a fresh `needs_confirmation` starts a new "attempt," and the previous attempt's own async work checks that it's still current before acting, so a manual Confirm/Cancel tapped mid-listen can't race with a decision the auto-listen path was about to make.

**Visible signal, on purpose, for the upcoming microphone-privacy security audit:** auto-listening shows a distinct ring around the mic button (`--color-good`, this app's existing "positive" token — never used elsewhere on this button, and deliberately different from the accent-colored pulse the manual press-and-hold recording already owns, so the two are never visually confusable) plus a matching text line in the result toast ("Listening — say yes or no…" / "Checking what you said…"). A family member should never have to wonder whether the mic just turned itself on — this feature is the one place in the app where the mic opens with literally no tap at all, so the "was that on?" question this raises gets an unambiguous visual answer every time, not just on first use.

**Testing:** `e2e/voice-auto-confirm.spec.ts`, same isolated-harness pattern as `voice.spec.ts`/`voice-command.spec.ts` (mocked `getUserMedia`/`MediaRecorder`, the mock Deepgram server, `e2e/mock-claude-cli.mjs`'s `MOCK_PROPOSE_DELETE:<id>` trigger) — covers a clear high-confidence "yes" really auto-confirming (the event is actually deleted, no manual tap anywhere in the test), a clear high-confidence "no" auto-cancelling (nothing deleted, no server call — matches the manual Cancel button's own stateless client-side-only behavior), a silent/empty second clip (standing in for "timed out with nothing usable") leaving the manual buttons up and still functional, a whitelist match paired with a *low* confidence value (via the mock server's `CONF:` prefix) also leaving the manual buttons untouched, and `getUserMedia` failing specifically on the *second* (auto-listen's own) call — after the *first* call, the manual recording's own permission prompt, already succeeded — degrading silently with the manual Confirm button still fully working afterward. A test-only `window.__AUTO_LISTEN_MAX_MS__` override (read once when the hook's module evaluates, same seam-for-substitution spirit as `deepgram.ts`'s `fetchImpl`/`claudeCli.ts`'s `spawnImpl`) keeps these tests fast without needing to wait out a real 10-second cap — real silence-detection timing (the `AnalyserNode` path) isn't exercised by this suite at all, since the mocked `getUserMedia` stream isn't a real `MediaStream` a real `AudioContext` can attach to, so every test here exercises the flat-timeout fallback path, not the early-stop-on-pause refinement specifically; that refinement's correctness rests on the standard, well-documented RMS-threshold technique rather than on its own dedicated test. Full suite: 116/116 passing (111 baseline + this work's 5 new tests), including two pre-existing §9 tests (`e2e/voice.spec.ts`) updated from an exact-object `toEqual` to field-specific assertions now that `POST /api/voice/transcribe`'s response carries the new `words` field alongside `transcript`.
...
- **M8a (v2) — Voice-driven auto-confirm. ✅ Done** (§10b). On top of M8's manual Confirm/Cancel, a `needs_confirmation` result now also auto-opens the mic (no new tap) for up to 10s, evaluating a spoken yes/no against a whitelist+confidence gate (`voiceConfirmWhitelist.ts`) before ever auto-firing the same `confirmAction`/`cancelAction` the manual buttons already call. Manual buttons stay up the entire time and are the only path forward on anything unclear. Programmatic `getUserMedia()`-with-no-new-gesture confirmed via MDN + this project's own e2e harness; real iOS Safari device behavior is the one still-open item, flagged explicitly in §10b.
